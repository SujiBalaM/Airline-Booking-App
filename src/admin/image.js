...
class ImageComponent extends Component {
  ...
  handleClickOpen(args) {
    ...
    const data = this.props.dashboardList.filter((response,{key:id}) => response.id === this.props.data.id );
    this.setState({ isOpen: true, asData: data[0] });
    }
  }
  clickOpen(){
        ...
        <Dialog open={this.state.show} onClose={this.clickClose}>
          ...
          <DialogContent>Are you sure you want to delete?</DialogContent>
            <DialogActions>
              ...
              <Button type='submit' name='submit' color='primary' onClick={this.clickClose} >
                Ok
              </Button>
            </DialogActions>
        </Dialog>
